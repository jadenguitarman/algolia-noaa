...
const stats = await request(`/1/indexes/${encodeURIComponent(indexName)}/stats`);
console.log(`Index ${indexName} seeded with ${stats.numberOfRecords} records. searchableAttributes=${JSON.stringify(settings.searchableAttributes)}`);
